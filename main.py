...
from games.crash import init_crash
...
# Если не нашли, используем прямой токен (временное решение)
if not BOT_TOKEN:
    print("⚠️ BOT_TOKEN не найден в переменных окружения, используем прямой токен")
    BOT_TOKEN = "8590626409:AAFhCW98PcsQFDOXNOC2l73cke-XDY7hfJc"  # Токен установлен
...
def normalize_text(text: str) -> str:
    ...
    # Убираем эмодзи (сохраняем буквы, цифры, пробелы, дефисы для диапазонов, точки и запятые)
...
async def handle_text(message: Message):
    ...
    # Команды рулетки
        ...
        if message.chat.type == "private":
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="🎮 Перейти в чат", url="https://t.me/chatikO1")]
                ]
            )
            await message.reply("🚫 Рулетку можно играть только в чатах!\n\n🎮 Перейдите в чат чтобы сделать ставку", reply_markup=keyboard)
            return
        ...
        try:
            ...
            if len(parts) >= 3:
                ...
                if user:
                    ...
                    if bet is not None:
                        if bet <= user['balance'] and bet > 0:
                            # Просто передаем все аргументы после ставки
                            bet_values = parts[2:]  # Все что после ставки
        ...
        if message.chat.type == "private":
            ...
            await message.reply("🚫 Рулетку можно играть только в чатах!\n\n🎮 Перейдите в чат чтобы запустить рулетку", reply_markup=keyboard)
            return
        ...
        if message.chat.type == "private":
            ...
            await message.reply("🚫 Историю рулетки можно смотреть только в чатах!", reply_markup=keyboard)
            return
    ...
    # Команды краш
        ...
        # Проверяем включена ли игра
        game_settings = await db.get_game_settings()
        if not game_settings.get("crash", True):
            await message.reply("🚫 Игра Краш временно отключена", reply_to_message_id=message.message_id)
            return
        
        if text == "краш" or text == "краш ":
            await crash_game.show_help(message)
        else:
            try:
                parts = text.split()
                if len(parts) < 3:
                    await message.reply("❌ Используйте: краш [ставка] [коэффициент от 1.0 до 10.0]")
                    return
                
                # Парсим ставку
                stake_str = parts[1].strip().lower()
                stake = parse_amount(stake_str, 0)
                if stake is None:
                ...
                # Парсим коэффициент
                try:
                    coeff_str = parts[2].strip()
                    
                    # Пробуем разные форматы
                    if '.' in coeff_str:
                        chosen_x = float(coeff_str)
                    elif ',' in coeff_str:
                        chosen_x = float(coeff_str.replace(',', '.'))
                    else:
                        chosen_x = float(coeff_str)
                except ValueError as e:
                    await message.reply("❌ Неверный формат коэффициента. Используйте точку или запятую.")
                    return
                
                await crash_game.start_game(message, stake, chosen_x)
            except Exception as e:
                await message.reply("❌ Ошибка при обработке команды. Попробуйте еще раз.")
                return
...
async def handle_top(message: Message):
    ...
    for i, user in enumerate(top_users, 1):
        ...
        # Добавляем кнопку для перехода на профиль с правильным форматом
        username = user.get('username')
        if username:
            profile_url = f"https://t.me/{username}"
        else:
            profile_url = f"https://t.me/user_{user['user_id']}"
...
async def handle_game_control(message: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text=f"🚀 Краш {'✅' if settings.get('crash', True) else '🚫'}", 
...
async def game_control_callback(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text=f"🚀 Краш {'✅' if updated_settings.get('crash', True) else '🚫'}", 
                    callback_data="game_crash"
                )
            ],
            [
...
🚀 Краш [ставка] [коэффициент] - ракета с множителем
